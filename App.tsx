              ...
              children: (
                <Form onFinish={handleRegister} layout="vertical">
                  ...
                  <Form.Item
                    ...
                    rules={[
                      ({ getFieldValue }) => {
                        if (getFieldValue('role') === 'student' && !getFieldValue('coachId')) {
    ...
    <Layout style={{ minHeight: '100vh' }}>
      <Sider width={220} theme="dark">
        ...
        <div style={{ 
          ...
          <Button type="text" danger icon={<LogoutOutlined />} onClick={logout} block>
            退出登录
          </Button>
        </div>
      </Sider>
...
const UploadPage: React.FC = () => {
  ...
  return (
    <div>
      <Row gutter={24}>
        <Col span={12}>
          <Card title="上传靶图" extra={<ReloadOutlined />}>
        ...
        <Col span={12}>
          <Card title="AI分析结果" extra={loadingAnalysis && <Spin size="small" />}>
            {analysisResult ? (
              <div>
                {analysisResult.record && (
                  <div style={{ marginBottom: 16 }}>
                    <Image 
                      src={`${BACKEND_URL}${analysisResult.record.url}`}
                ...
                {analysisResult.analysis && (
                  <div>
                    ...
                    {analysisResult.analysis.overall_assessment?.comprehensive_score && (
                      <div style={{ textAlign: 'center', marginBottom: 16 }}>
                        <Text strong>综合评分：</Text>
                        <Tag color="green" style={{ fontSize: 18 }}>
                          ⭐ {analysisResult.analysis.overall_assessment.comprehensive_score}/10
                        </Tag>
                      </div>
                    )}
...
const HistoryPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (searchText) {
      filtered = filtered.filter(r => 
        ...
        (r.username && r.username.toLowerCase().includes(searchText.toLowerCase()))
      );
    }
  ...
  const exportToCSV = () => {
    const headers = ['文件名', '用户名', '上传时间', '环数', '评分'];
    ...
    link.setAttribute('download', `射击训练记录_${dayjs().format('YYYYMMDD')}.csv`);
    ...
    <div>
      <div style={{ marginBottom: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          ...
          <Space>
            <Button icon={<DownloadOutlined />} onClick={exportToCSV} disabled={filteredRecords.length === 0}>
              导出CSV
            </Button>
            <Button icon={<ReloadOutlined />} onClick={fetchAllRecords} loading={loading}>
              刷新
            </Button>
          </Space>
        </div>
        ...
        <Card size="small" style={{ marginBottom: 16 }}>
          <Space wrap>
            ...
            />
            {(searchText || minRing !== null || (dateRange && dateRange.length > 0)) && (
              <Button onClick={() => {
                setSearchText('');
                setMinRing(null);
                setDateRange([]);
                ...
                <List.Item>
                  ...
                  >
                    <Card.Meta
                      title={
                        <div>
                          <Text ellipsis>{item.originalFilename}</Text>
                          {item.username && (
              ...
              <List.Item>
                <Card>
                  <Card.Meta
                    ...
                    description={
                      <div>
                        <div><Text type="secondary">注册时间：{dayjs(student.createdAt).format('YYYY-MM-DD')}</Text></div>
                        <Tag color="green" style={{ marginTop: 8 }}>学员</Tag>
                      </div>
                    }
                  />
                </Card>
...
const StatsPage: React.FC = () => {
  ...
  const avgRing = records.filter(r => r.analysis?.metadata?.hit_ring).length > 0 
    ? (records.filter(r => r.analysis?.metadata?.hit_ring)
        .reduce((sum, r) => sum + (r.analysis.metadata.hit_ring || 0), 0) / 
        records.filter(r => r.analysis?.metadata?.hit_ring).length).toFixed(1)
    : 0;
  const avgScore = records.filter(r => r.analysis?.overall_assessment?.comprehensive_score).length > 0 
    ? (records.filter(r => r.analysis?.overall_assessment?.comprehensive_score)
        .reduce((sum, r) => sum + (r.analysis.overall_assessment.comprehensive_score || 0), 0) / 
        records.filter(r => r.analysis?.overall_assessment?.comprehensive_score).length).toFixed(1)
    : 0;

  // 环数分布图表
  const getRingChartOption = () => {
    const ringData: Record<string, number> = {
      '10-10.9': 0, '9-9.9': 0, '8-8.9': 0, '7-7.9': 0, '6-6.9': 0, '5以下': 0
    };
  ...
  const getTrendChartOption = () => {
    ...
    records.filter(r => r.analysis?.metadata?.hit_ring).forEach(r => {
      ...
      if (!dateData[date]) {
        dateData[date] = { count: 0, totalRing: 0 };
      }
      dateData[date].count++;
    ...
    const avgRingData = sortedDates.map(date => 
      (dateData[date].totalRing / dateData[date].count).toFixed(1)
    );
      ...
      <Spin spinning={loading}>
        ...
        {records.length === 0 ? (
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Card title="环数分布">
                <ReactECharts option={getRingChartOption()} style={{ height: 400 }} />
              </Card>
            </Col>
            <Col span={12}>
              <Card title="平均环数趋势">
