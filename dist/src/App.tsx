...
const MainApp: React.FC = () => {
  ...
  return (
    <Layout style={{ minHeight: '100vh' }}>
      ...
      <Layout>
        <Header style={{ background: '#fff', padding: '0 24px', display: 'flex', alignItems: 'center', boxShadow: '0 1px 4px rgba(0,0,0,0.1)' }}>
            ...
            {analysisResult ? (
              <div>
                {analysisResult.record && (
                  <Image src={`${BACKEND_URL}${analysisResult.record.url}`} style={{ width: '100%', maxHeight: 300, objectFit: 'contain' }} />
                )}
            ...
            <List
              ...
              renderItem={(item) => (
                <List.Item>
                  <Card
                    hoverable
                    cover={<Image src={`${BACKEND_URL}${item.url}`} preview={false} style={{ height: 200, objectFit: 'cover' }} onClick={() => setPreviewImage(`${BACKEND_URL}${item.url}`)} />}
                    actions={[
